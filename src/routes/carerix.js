...
router.get('/inspect-login', async (req, res) => {
  ...
  for (const show of showVariants) {
    ...
    try {
      ...
      results[`show=${show || '(none)'}`] = { error: e.message, status: e.response?.status };
    }
  }
  const crUserId = parseXml(
    (await axios.get(`${restBase}CRUser/login-with-encrypted-password`, { params: { u, p }, headers, timeout: 10_000, responseType: 'text' }).catch(() => ({ data: '' }))).data
  )?.CRUser?.['@_id'];
  if (crUserId) {
    for (const show of ['', 'groups', 'toEmployee,toContact,toCompany,toUserRole,userRoleID']) {
      ...
      if (show) params.show = show;
      try {
        const r = await axios.get(`${restBase}CRUser/${crUserId}`, { params, headers, timeout: 8_000, responseType: 'text' });
        results[`CRUser/${crUserId} show=${show || '(none)'}`] = { raw: r.data?.substring(0, 1000), parsed: parseXml(r.data) };
      } catch (e) {
        results[`CRUser/${crUserId} show=${show || '(none)'}`] = { error: e.message };
      }
    }
    try {
      const r = await axios.get(`${restBase}CREmployee`, { params: { qualifier: `toUser._id = ${crUserId}`, show: 'groups', limit: 1 }, headers, timeout: 8_000, responseType: 'text' });
...
// ── GET /carerix/invoice-data/:jobId ──────────────────────────────────────────
router.get('/invoice-data/:jobId', async (req, res, next) => {
  try {
    ...
    let employee = null;
    if (empId) {
      const empResult = await queryGraphQL(`query EmpDetail($id: ID!) { crEmployee(_id: $id) { _id employeeID firstName lastName name paymentIbanCode paymentBicCode paymentAccountName homeFullAddress homeStreet homeNumber homeNumberSuffix homePostalCode homeCity toHomeCountryNode { value } } }`, { id: String(empId) });
      employee = empResult?.data?.crEmployee;
    }
    const compId = job.toCompany?._id;
    let company = null;
    if (compId) {
      const compResult = await queryGraphQL(`query CompDetail($id: ID!) { crCompany(_id: $id) { _id companyID name visitCity visitPostalCode visitStreet visitNumber toVisitCountryNode { value } emailAddress phone vatNumber } }`, { id: String(compId) });
...
// ── GET /carerix/employee-finances/:employeeId ────────────────────────────────
router.get('/employee-finances/:employeeId', async (req, res, next) => {
  try {
    const { queryGraphQL } = await import('../services/carerix.js');
    const { employeeId } = req.params;
    const result = await queryGraphQL(`query EmployeeFinancePage($qualifier: String, $pageable: Pageable) { crEmployeeFinancePage(qualifier: $qualifier, pageable: $pageable) { items { _id toFinance { _id startDate endDate amount cost info toKindNode { dataNodeID value } toCurrencyNode { dataNodeID value } toTypeNode { typeID identifier } } } } }`, { qualifier: 'toEmployee.employeeID == ' + employeeId, pageable: { page: 0, size: 100 } });
    res.json(result);
  } catch (err) { next(err); }
});

// ── GET /carerix/job-finances/:jobId ──────────────────────────────────────────
router.get('/job-finances/:jobId', async (req, res, next) => {
  try {
    const { queryGraphQL } = await import('../services/carerix.js');
    const { jobId } = req.params;
    const result = await queryGraphQL(`query JobFinancePage($qualifier: String, $pageable: Pageable) { crJobFinancePage(qualifier: $qualifier, pageable: $pageable) { items { _id toJob { _id jobID name } toFinance { _id startDate endDate amount cost info toKindNode { dataNodeID value } toCurrencyNode { dataNodeID value } toTypeNode { typeID identifier } } } } }`, { qualifier: 'toJob.jobID == ' + jobId, pageable: { page: 0, size: 100 } });
    res.json(result);
  } catch (err) { next(err); }
});
...
router.get('/probe', async (req, res, next) => {
  try {
    ...
    try { const r = await queryGraphQL('query J($id: ID!) { crJob(_id: $id) { _id jobID name } }', { id: '5319' }); results.crJob_var = r?.data?.crJob || r?.errors?.[0]?.message || 'null'; } catch(e) { results.crJob_var = 'ERROR: ' + e.message; }
    try { const r = await queryGraphQL('query { crJob(_id: "5319") { additionalInfo additionalInfoList } }', {}); results.additionalInfo = r?.data?.crJob || r?.errors?.[0]?.message || 'null'; } catch(e) { results.additionalInfo = 'ERROR: ' + e.message; }
    try { const r = await queryGraphQL('query { crEmployee(_id: "14") { _id employeeID firstName lastName homeCity homePostalCode paymentIbanCode paymentBicCode paymentAccountName } }', {}); results.employee14 = r?.data?.crEmployee || r?.errors?.[0]?.message || 'null'; } catch(e) { results.employee14 = 'ERROR: ' + e.message; }
...
// No auth required. Shows exactly what Carerix returns for employee identifier
// fields, and what buildCrewCodeToJobMap produces.
router.get('/crew-code-probe', async (req, res, next) => {
  try {
    const { queryGraphQL, buildCrewCodeToJobMap } = await import('../services/carerix.js');
    const out = {};

    // 1. Sample 5 employees — all identifier fields we try
    try {
      const r = await queryGraphQL(`
        query {
          crEmployeePage(pageable: { page: 0, size: 5 }) {
            totalElements
            items { _id employeeID firstName lastName code1 code2 number }
          }
        }
      `, {});
      out.employee_sample = r?.data?.crEmployeePage ?? r?.errors ?? 'no data';
    } catch(e) { out.employee_sample = 'ERROR: ' + e.message; }

    // 2. Sample 5 jobs with linked employee identifier fields
    try {
      const r = await queryGraphQL(`
        query {
          crJobPage(pageable: { page: 0, size: 5 }) {
            ...
            items {
              _id jobID name
              toEmployee { _id employeeID firstName lastName code1 code2 number }
            }
          }
        }
      `, {});
    ...
    // 3. Look at ERIC's job specifically to understand which field holds "ERIC"
    try {
      const r = await queryGraphQL(`
        query {
          crJob(_id: "5319") {
    ...
    // 4. Run the actual map builder and show what it found
    try {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (result?.retrieval_status === 'retrieved') {
        await adminSupabase.from('declaration_entries').update({ fee_cache_id: result.id, fee_amount: result.fee_amount, fee_retrieval_status: 'retrieved', calculated_value: entry.imported_amount * result.fee_amount, status: 'fee_retrieved' }).eq('id', entry.id);
        retrieved++;
      } else {
        await adminSupabase.from('declaration_entries').update({ fee_retrieval_status: 'failed', status: 'fee_retrieval_failed' }).eq('id', entry.id);
        failed++;
      }
    }
...
// ── GET /carerix/placement-rates/:carerixPlacementId ─────────────────────────
