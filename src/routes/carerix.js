...
router.get('/invoice-data/:jobId', async (req, res, next) => {
  try {
      ...
      const empResult = await queryGraphQL(`
        query EmpDetail($id: ID!) {
          crEmployee(_id: $id) {
            ...
            homeFullAddress homeStreet homeNumber homeNumberSuffix homePostalCode homeCity
    ...
    if (typeof rawAI === 'object') {
      for (const [k, v] of Object.entries(rawAI)) { ai[k.replace(/^_/, '')] = v; }
    }
    ...
    const result = await queryGraphQL(`
      query EmployeeFinancePage($qualifier: String, $pageable: Pageable) {
        crEmployeeFinancePage(qualifier: $qualifier, pageable: $pageable) {
          items { _id toFinance { _id startDate endDate amount cost info toKindNode { dataNodeID value } toCurrencyNode { dataNodeID value } toTypeNode { typeID identifier } } }
        }
      }
    `, { qualifier: 'toEmployee.employeeID == ' + employeeId, pageable: { page: 0, size: 100 } });
    res.json(result);
  } catch (err) { next(err); }
});
    ...
    const result = await queryGraphQL(`
      query JobFinancePage($qualifier: String, $pageable: Pageable) {
        crJobFinancePage(qualifier: $qualifier, pageable: $pageable) {
          items { _id toJob { _id jobID name } toFinance { _id startDate endDate amount cost info toKindNode { dataNodeID value } toCurrencyNode { dataNodeID value } toTypeNode { typeID identifier } } }
        }
      }
    `, { qualifier: 'toJob.jobID == ' + jobId, pageable: { page: 0, size: 100 } });
    res.json(result);
  } catch (err) { next(err); }
});
...
router.get('/rate-table/:id', requireAuth, requireAgency, async (req, res, next) => {
  try {
    const { queryGraphQL } = await import('../services/carerix.js');
    const result = await queryGraphQL(`
      query RateTable($id: ID!) { crRateTable(id: $id) { _id name description crRateTableLines { _id amount validFrom validUntil description } } }
    `, { id: req.params.id });
    res.json(result);
  } catch (err) { next(err); }
});
    ...
    const result = await queryGraphQL(`
      query PlacementRates($id: ID!) {
        crMatch(id: $id) {
          ...
          crMatchConditions { _id amount toCRRateTable { _id name crRateTableLines { _id amount validFrom validUntil description } } }
        }
      }
